...
class RabbitMQMixin:
    ...
    def setup_rabbitmq(self, crawler):
        ...
        if self.max_idle_time is None:
            self.max_idle_time = crawler.settings.get(
                "MAX_IDLE_TIME_BEFORE_CLOSE", defaults.MAX_IDLE_TIME_BEFORE_CLOSE
            )
        ...
        self.server = connection.RabbitmqManager(
            crawler.settings.get("RABBITMQ_CONNECTION_URL", defaults.RABBITMQ_CONNECTION_URL),
            crawler.settings.get("RABBITMQ_AUTO_ACK", defaults.RABBITMQ_AUTO_ACK),
            crawler.settings.get("RABBITMQ_DURABLE", defaults.RABBITMQ_DURABLE),
            crawler.settings.get("RABBITMQ_PREFETCH_COUNT", defaults.RABBITMQ_PREFETCH_COUNT),
            crawler.settings.get("RABBITMQ_CONFIRM_DELIVERY", defaults.RABBITMQ_CONFIRM_DELIVERY),
            self.queue_name,
        )
